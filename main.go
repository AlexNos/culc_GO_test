...
func RomanianToArabicExist(x string) bool {
	_, exists := mapR2A[x]
	return exists
}
...
func main() {
	...
	var arg3 int
	var result int
	rom := false // флаг того, что вводятся римские числа
	...
	if len(elements) > 3 {
		panic("Ошибка введения данных: количество операндов должно быть 2")
	}

	for i, elem := range elements {

		if i == 0 {
			if RomanianToArabicExist(elem) {
				arg1 = RomanianToArabic(elem)
				if arg1 > 10 {
					panic("Ошибка ввода данных! Числа должны быть не более X")
				}
				rom = true
			} else {
		...
		if i == 2 {
			if RomanianToArabicExist(elem) {
				if !rom {
					panic("Ошибка введения данных (оба числа должны быть римские или арабские одновременно)")
				}
				if arg1 > 10 {
					panic("Ошибка ввода данных! Числа должны быть не более X")
				}
	...
	if rom {
		...
		fmt.Println(ArabicToRomanian(mapR2A, result))
	} else {
		fmt.Println(result)
	}
}
